...
const slackNamesToDisplayNames = new Map(Object.entries(SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_NAME_DISPLAY_NAMES).getDataRange().getValues().slice(1)));
...
const doGet = (event) => {
    const data = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Rankingtabell').getDataRange().getValues().slice(1).map(row => ({
        navn: row[0],
        poeng: row[1]
    })).map((row, index) => ({
        ...row,
